...
const GameEngine: React.FC<GameEngineProps> = ({
    ...
    useEffect(() => {
        if (dropAction) {
            ...
            const { cx, cy } = getChunkCoords(dropX, dropY);
            const key = getChunkKey(cx, cy);

            if (worldRef.current.chunks[key]) {
                worldRef.current.chunks[key].droppedItems.push({
                    id: `drop-${Date.now()}-${Math.random()}`,
                    type: dropAction.type,
                    count: dropAction.count,
                    x: dropX,
                    y: dropY,
                    durability: dropAction.durability,
                    maxDurability: dropAction.maxDurability,
                    contents: dropAction.contents,
                    pickupDelay: 1.0, // 1 second delay before pickup
                    floatOffset: Math.random() * Math.PI * 2,
                    lifeTime: 300
                });
                onStatusUpdate(`Dropped ${ITEM_NAMES[dropAction.type]}`);
            }
