...
interface FullInventoryProps {
    ...
    onDropCursor: () => void;
    onDropFromCursor: (count: number) => void;
    onDropStack: (context: 'player' | 'container' | 'equip' | 'backpack', index: number | string) => void;
}
...
const FullInventory: React.FC<FullInventoryProps> = ({
    ...
    health, maxHealth, stamina, maxStamina, equipment, cursorStack, onDropCursor, onDropFromCursor, onDropStack
}) => {
    const [hoverItem, setHoverItem] = useState<{ item: InventoryItem, x: number, y: number, context: 'player' | 'container' | 'equip' | 'backpack', index: number | string } | null>(null);
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key.toLowerCase() === 'q') {
                if (hoverItem) {
                    onDropStack(hoverItem.context, hoverItem.index);
                    setHoverItem(null); // Clear hover to prevent spamming
                }
            }
        };
        window.addEventListener('keydown', handleKeyDown);
        return () => window.removeEventListener('keydown', handleKeyDown);
    }, [isOpen, hoverItem, onDropStack]);
    ...
    const handleMouseMove = (e: React.MouseEvent, item: InventoryItem | null, context: 'player' | 'container' | 'equip' | 'backpack', index: number | string) => {
        if (item && !cursorStack) {
            setHoverItem({ item, x: e.clientX, y: e.clientY, context, index });
        } else {
            setHoverItem(null);
        }
    };
    ...
    const renderSlot = (item: InventoryItem | null, index: number, context: 'player' | 'container' | 'backpack') => (
        <div
            ...
            onMouseMove={(e) => handleMouseMove(e, item, context, index)}
    ...
    const renderEquipSlot = (slot: EquipmentSlot, icon: React.ReactNode) => {
        ...
        return (
            <div
                ...
                onMouseMove={(e) => handleMouseMove(e, item, 'equip', slot)}
                onMouseLeave={() => setHoverItem(null)}
